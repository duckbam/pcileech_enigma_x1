...
module pcileech_fifo #(
    ...
    always @ ( posedge clk )
        ...
        // Anti-Detection: Hardware timing randomization
    reg [31:0] timing_randomizer = 32'h12345678;
    reg [15:0] response_delay = 0;
    reg [7:0]  random_seed = 8'hA5;
    
    always @ ( posedge clk ) begin
        timing_randomizer <= timing_randomizer ^ (timing_randomizer << 13);
        timing_randomizer <= timing_randomizer ^ (timing_randomizer >> 17);
        timing_randomizer <= timing_randomizer ^ (timing_randomizer << 5);
        response_delay <= timing_randomizer[15:0] % 8 + 1;
        if (tickcount64[23:0] == 0) begin
            random_seed <= random_seed + timing_randomizer[7:0];
        end
    end
